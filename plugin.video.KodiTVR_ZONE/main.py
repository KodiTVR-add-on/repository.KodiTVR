...
##CONFIGURAÇÕES
####  TITULO DO MENU  #################################################################
title_menu = "[B][COLORlime]●[/COLOR][/B] [COLORblue]TV  [COLORgold]ONLINE[/COLOR]  [COLORred]KodiTVR[/COLOR]  [B][COLORred]●[/COLOR][/B]"
###  DESCRIÇÃO DO ADDON ###############################################################
title_descricao = "KodiTVR - IPTV"
####  LINK DO TITULO DE MENU  #########################################################
url_title = 'AQUI ENCONTRA O ADD-ON - KodiTVR TV ONLINE PARA O APLICATIVO KODI'

#### MENSAGEM BEM VINDOS  #############################################################
titulo_boas_vindas = "BEM-VINDOS"
mensagem_bem_vindo = "[B][COLOR green]Agradeço por terem escolhido o add-on[/COLOR] [COLOR red]KodiTVR[/COLOR]\n[COLOR gold]Instale:[/COLOR] [COLOR blue]repository.KodiTVR[/COLOR] [COLOR gold](DOWNLOAD)[/COLOR] https://KodiTVR-add-on.github.io[/B]"
####  MENSAGEM SECUNDARIA ######################################################
mensagem_secundaria = "[COLOR gold][B]KodiTVR[/B][/COLOR] - [COLOR white][B]ADD-ONS, BY KODITVR[/B][/COLOR]"
...
thumb_icon_config = 'https://i.imgur.com/XLKBDok.jpg'
desc_configuracoes = "Configurações do KodiTVR"

## FAVORITOS
menu_favoritos = "[B][COLOR white]FAVORITOS[/COLOR][/B]"
thumb_favoritos = 'https://i.imgur.com/kp197rn.jpg'
desc_favoritos = "Adicione Itens aos Favoritos, pressionando OK do controle "

#### MENU VIP ################################################################
titulo_vip = "[B][COLOR lime]KodiTVR[/COLOR]  [COLOR gold](IPTV - VIP)[/COLOR][/B]"
thumbnail_vip = 'https://i.imgur.com/d5EoSpO.jpg'
fanart_vip = 'https://i.imgur.com/d5EoSpO.jpg'
#### DESCRIÇÃO VIP ###########################################################
vip_descricao = 'VIP, KodiTVR - CHANNELS  [B][COLORred]●[/COLOR][/B]'
#### DIALOGO VIP - SERVIDOR DESATIVADO - CLICK ###################################
vip_dialogo = "[B][COLOR white]CONFIGURE SEU ACESSO A NOSSA ÁREA VIP ANTES DE ENTRAR AQUI, MAIS INFORMAÇÕES ENTRE EM CONTACTO COM O NOSSO SUPORTE\nENTRE EM CONTACTO PELO SITE: https://koditvr.wixsite.com/website[/COLOR][/B]"
##SERIVODR VIP
url_server_vip1 = ''
url_server_vip2 = ''
...
#Base
url_b64_principal = 'aHR0cHM6Ly9naG9zdGJpbi5jby9wYXN0ZS9qa3l4Ny9yYXc='
...
def rc_pro3(channel):
    try:
        ...
        regex = getRequest2('https://dl.dropboxusercontent.com/s/teu0aese4l9hf3e/rc_pro.txt','').replace('\n','').replace('\r','')
...
def addFavorite(name,url,fav_mode,subtitle,cleaname,iconimage,fanart,description):
    ...
    if os.path.exists(favorites)==False:
        ...
        a.close()
        notify('Adicionado aos Favoritos do KodiTVR_ZONE')
...
def rmFavorite(name):
    ...
    for index in range(len(data)):
        if data[index][0]==name:
            ...
            b.close()
            break
    notify('Removido dos Favoritos do KodiTVR_ZONE')
    xbmc.executebuiltin("XBMC.Container.Refresh")            
...
def addDir(name,url,mode,iconimage,fanart,description,folder=True):
    ...
    if favoritos == 'true' and  mode !=4 and mode !=7 and mode !=8 and mode !=9 and mode !=10 and mode !=15 and not url.find('username') >= 0:
        ...
        try:
            ...
            if name_fav in FAV:
                try:
                    contextMenu.append(('Remover dos Favoritos do KodiTVR_ZONE','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name))))
                except:
                    contextMenu.append(('Remover dos Favoritos do KodiTVR_ZONE','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name.encode('utf-8', 'ignore')))))
            else:
                try:            
                ...
                contextMenu.append(('Adicionar aos Favoritos do KodiTVR_ZONE','XBMC.RunPlugin(%s)' %fav_params))
...
def addLink(name,url,subtitle,cleaname,iconimage,fanart,description,folder=False):
    ...
    if favoritos == 'true':
        try:
            ...
            if name_fav in FAV:
                try:
                    contextMenu.append(('Remover dos Favoritos do KodiTVR_ZONE','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name2_fav))))
                except:
                    contextMenu.append(('Remover dos Favoritos do KodiTVR_ZONE','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name2_fav.encode('utf-8', 'ignore')))))
            else:
                try:           
    ...
    try:
        ...
        if exists == True:
            ...
            if fcheck and fcheck.read() == '1' and check_addon == 'true':
                #print('valor 1')
                fcheck.close()
                link = getRequest2('https://raw.githubusercontent.com/KodiTVR-add-on/iptv/main/verificar_addons.txt','').replace('\n','').replace('\r','')
...
def SKindex():
    ...
    if addon.getSetting('mensagem2') == 'true':
        ...
        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,getRequest2('https://pastebin.com/raw/Pz5tSxcj', 'https://pastebin.com/raw/Pz5tSxcj'), time_msg, __icon__))
