...
def mensagem():
    ...
    #dialog.notification('[COLORred] Entrou no KodiTVR-ADULT-XXL *ONLINE* NOVO ADD-ON... [/COLOR] ', LINE1, xbmcgui.NOTIFICATION_INFO, 10000)
    getSources()
    xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(addon1,LINE1, 10000, icon))
    CHIndex()
        ...
        for item in items:
            ...
            try:
                ...
                if len(item('link')) >0:
                    ...
                    for i in item('imdb'):
                        if not i.string == None:
                            if addon.getSetting('genesisorelementum') == '0':
                                imdb = 'plugin://plugin.video.genesis/?action=play&imdb='+i.string
                            else:
                                imdb = 'plugin://plugin.video.elementum/play?uri='+i.string+'/play'
                            url.append(imdb)
                elif len(item('f4m')) >0:
                        for i in item('f4m'):
                            if not i.string == None:
