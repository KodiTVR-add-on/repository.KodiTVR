...
def update_players_local(zip_path):
	extract_to = xbmc.translatePath('special://profile/addon_data/plugin.video.KodiTVR_VIDEO_PLAYER/Players')  
...
def flat_extract(z, extract_to, members=None):
	...
	if not os.path.exists(extract_to):
		...
		if xbmc.getInfoLabel('Window(home).Property(running)') == 'totalKodiTVR_VIDEO_PLAYER':
			empty_folder(extract_to)
		else:
			if plugin.yesno('KodiTVR_VIDEO_PLAYER: Update players', 'Do you want to remove existing players first?'):
				empty_folder(extract_to)
