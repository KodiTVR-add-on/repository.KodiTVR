...
def movies_add_all_to_library(items, noscan=False):
	...
	batch_add_file.close()
	xbmc.executebuiltin('RunPlugin(plugin://plugin.video.KodiTVR_VIDEO_PLAYER/movies/batch_add_to_library)')
...
def movies_play(src, id, usedefault='True'):
	...
	if playaction == 1 and xbmc.getCondVisibility('system.hasaddon(script.KodiTVRinfo)') and not plugin.getProperty('infodialogs.active'):
		...
		xbmc.executebuiltin('RunScript(script.KodiTVRinfo,info=KodiTVRinfo,%s=%s)' % (src, id))
	else:
...
def make_movie_item(movie_info):
	...
	if xbmc.getCondVisibility('system.hasaddon(script.KodiTVRinfo)'):
		context_menu = [
			('Movie trailer', 'RunScript(script.KodiTVRinfo,info=playtrailer,id=%s)' % id),
