...
def tv_play(id, season, episode, usedefault='True'):
	...
	if playaction == 1 and xbmc.getCondVisibility('system.hasaddon(script.KodiTVRinfo)') and not plugin.getProperty('infodialogs.active'):
		from resources.lib.play_base import action_cancel
		action_cancel()
		xbmc.executebuiltin('RunScript(script.KodiTVRinfo,info=extendedtvinfo,tvdb_id=%s)' % (id))
	elif playaction == 2 and xbmc.getCondVisibility('system.hasaddon(script.KodiTVRinfo)') and not plugin.getProperty('infodialogs.active'):
		from resources.lib.play_base import action_cancel
		action_cancel()
		xbmc.executebuiltin('RunScript(script.KodiTVRinfo,info=extendedepisodeinfo,tvdb_id=%s,season=%s,episode=%s)' % (id, season, episode))
	else:
		play_tvshows.play_episode(id, season, episode, usedefault)
...
def tv_add_to_library_parsed(id, player):
	...
	if lib_tvshows.add_tvshow_to_library(library_folder, show, player):
		plugin.setProperty('plugin.video.KodiTVR_VIDEO_PLAYER.clean_library', 'true')
...
def tv_add_all_to_library(items, noscan=False):
	...
	batch_add_file.close()
	xbmc.executebuiltin('RunPlugin(plugin://plugin.video.KodiTVR_VIDEO_PLAYER/tv/batch_add_to_library)')
...
def make_tvshow_item(info):
	...
	if xbmc.getCondVisibility('system.hasaddon(script.KodiTVRinfo)'):
		context_menu = [
			('KodiTVRinfo', 'RunScript(script.KodiTVRinfo,info=extendedtvinfo,tvdb_id=%s)' % tvdb_id),
			('TV trailer', 'RunScript(script.KodiTVRinfo,info=playtvtrailer,tvdb_id=%s)' % tvdb_id),
...
def list_seasons_tvdb(id, flatten):
	...
	for (season_num, season) in show.items():
		...
		if flatten == 'none':
			if xbmc.getCondVisibility('system.hasaddon(script.KodiTVRinfo)'):
				context_menu = [
					('KodiTVRinfo', 'RunScript(script.KodiTVRinfo,info=seasoninfo,tvshow=%s,season=%s)' % (show_info['name'], season_num))]
			else:
				context_menu = []
			properties = {}
			try:
