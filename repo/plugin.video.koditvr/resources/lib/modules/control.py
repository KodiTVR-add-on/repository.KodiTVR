...
def artPath():
    ...
    if theme in ['-', '']:
        return
    elif condVisibility('System.HasAddon(script.koditvr.artwork)'):
        return os.path.join(xbmcaddon.Addon(
            'script.koditvr.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting(
        'appearance.1').lower() if condVisibility(
        'System.HasAddon(script.koditvr.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.koditvr.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.koditvr.metadata)'):
        return os.path.join(xbmcaddon.Addon(
            'script.koditvr.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
